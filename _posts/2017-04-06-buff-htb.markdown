...
No es posible apuntar directamente a una ruta del stack, pero si a un offcode que haga el jump al ESP. Pero antes, conviene saber que caracteres no va a interpretar para agregarlos a una lista de badchars. Utilo ```mona.py```, que es un addon que se puede instalar en el ```Inmunity Debbuger```
